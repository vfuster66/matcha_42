...
exports.FameRatingService = void 0;
const database_1 = require("../config/database");
class FameRatingService {
    // Calcul du fame rating basé sur plusieurs critères
    static async calculateFameRating(userId) {
        try {
    ...
    static async updateFameRating(userId) {
        ...
        await database_1.db.query('UPDATE profiles SET fame_rating = $1 WHERE user_id = $2', [newRating, userId]);
    }
    // Obtention du fame rating actuel
    static async getFameRating(userId) {
        const result = await database_1.db.query('SELECT fame_rating FROM profiles WHERE user_id = $1', [userId]);
        return result.rows[0]?.fame_rating || 50;
    }
}
exports.FameRatingService = FameRatingService;
FameRatingService.MAX_RATING = 100;
FameRatingService.MIN_RATING = 0;
